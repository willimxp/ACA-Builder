...
class ACA_data_scene(bpy.types.PropertyGroup):
    is_auto_redraw : bpy.props.BoolProperty(
            default = True,
            name = _("是否实时重绘"),
            description = _("取消后，生成过程中不进行刷新，直到全部生成后才显示"),
        ) # type: ignore
    is_auto_viewall : bpy.props.BoolProperty(
            default = True,
            name = _("是否设置视角"),
            description = _("取消后，不再自动切换视角，始终保持当前视角"),
        ) # type: ignore
    is_auto_rebuild : bpy.props.BoolProperty(
            default = True,
            name = _("是否实时重建"),
            description = _("取消后，在大部分参数修改时，不会自动重建，直到手工点击更新建筑"),
        ) # type: ignore
...
class ACA_data_taduo(bpy.types.PropertyGroup):
    ...
    width : bpy.props.FloatProperty(
        name=_('踏跺宽度'),
        description=_('踏跺在开间内的比例，最大为1，最小为0.3'),
        default=1.0,
        max=1.0,
        min=0.3,
...
class ACA_data_railing(bpy.types.PropertyGroup):
    ...
    gap : bpy.props.FloatProperty(
        name=_('开口宽度'),
        description=_('在开间内开口的比例，设置为0时不做开口，最大为0.9'),
        default=0.0,
        max=0.9,
        min=0.0,
        step=10,    # 这里是[n/100],10代表0.1
...
class ACA_data_wall_common(bpy.types.PropertyGroup):
    ...
    wall_span : bpy.props.FloatProperty(
            ...
            min=0,
            precision=3,
            description=_('重檐时，装修不做到柱头，用走马板填充，输入0则不做走马板'),
...
class ACA_data_door_common(ACA_data_wall_common):
    doorFrame_width_per : bpy.props.FloatProperty(
            ...
            min=0.1,
            precision=3,
            description=_('开间中的门口/窗口宽度比例，小于1则开间的部分做余塞板，不可大于1'),
            update=dc.update_wall,
        )# type: ignore 
    doorFrame_height : bpy.props.FloatProperty(
            ...
            min=0.1,
            precision=3,
            description=_('开间中的门口高度，小于柱高的空间将自动布置横披窗/迎风板'),
            update=dc.update_wall,
        )# type: ignore 
    topwin_height : bpy.props.FloatProperty(
            ...
            precision=3,
            update=dc.update_wall,
            description=_("横披窗（棂心）的高度，输入0则不做横披窗"),
        )# type: ignore 
...
class ACA_data_maindoor(ACA_data_door_common):
    ...
    door_ding_num : bpy.props.IntProperty(
            ...
            min=0,max=9,
            update=dc.update_wall,
            description=_("门钉的路数，最大9路，取0时不做门钉"),
        )# type: ignore 
...
class ACA_data_geshan(ACA_data_door_common):
    door_num : bpy.props.IntProperty(
            ...
            update=dc.update_wall,
            description=_("一般做4扇隔扇"),
        )# type: ignore 
    gap_num : bpy.props.IntProperty(
            name=_("抹头数量"),
            default=5,min=2,max=6,
            update=dc.update_wall,
            description=_("2~6抹头都可以，根据需要自由设置"),
        )# type: ignore 
...
class ACA_data_obj(bpy.types.PropertyGroup):
    ...
    combo_children: bpy.props.CollectionProperty(
        type=ACA_id_list, name=_("组合关联子对象")
    ) # type: ignore
    ...
    DK: bpy.props.FloatProperty(
            ...
            step=0.01,
            precision=3,
            description=_("比例模数(m)，清官式常用0.08(二寸半)、0.096(三寸)等"),
            update=dc.update_dk
        ) # type: ignore
    ...
    platform_extend : bpy.props.FloatProperty(
            ...
            min = 0.01, 
            update=dc.update_platform,    # 绑定回调
            description=_("檐柱的2.4倍，或上出檐的3/4~4/5"),
        ) # type: ignore
    ...
    step_list: bpy.props.CollectionProperty(
        type=ACA_data_taduo, name=_("踏跺列表")
    ) # type: ignore
    ...
    x_rooms : bpy.props.IntProperty(
            ...
            description=_("必须为奇数，建议最多不超过11间"),
        )# type: ignore
    x_1 : bpy.props.FloatProperty(
            ...
            description=_("常取7攒斗栱，且一般柱不越间广（柱高小于明间宽度）"),
        )# type: ignore
    x_2 : bpy.props.FloatProperty(
            ...
            precision=3,
            update=dc.update_building,
            description=_("常取6攒斗栱，且一般柱不越间广（柱高小于明间宽度）"),
        )# type: ignore
    ...
    y_rooms : bpy.props.IntProperty(
            ...
            min = 1, 
            update=dc.reset_building,
            description=_("根据通进深的需要，以及是否做前后廊，可以为偶数"),
        )# type: ignore
    ...
    pillar_height : bpy.props.FloatProperty(
            ...
            description=_("有斗拱的取57-60斗口，无斗拱的取面阔的8/10"),
        )# type: ignore
    pillar_diameter : bpy.props.FloatProperty(
            ...
            update=dc.update_building,
            description=_("有斗拱的取6斗口，无斗拱的取1/10柱高"),
        )# type: ignore
    use_smallfang: bpy.props.BoolProperty(
            default=False,
            name=_("双重额枋"),
            update=dc.update_building,
            description=_("同时使用大额枋、由额垫板、小额枋的三件套连接两根柱"),
        )# type: ignore 
    pillar_insert: bpy.props.FloatProperty(
            ...
            min = 0.01, 
            precision=3,
            description=_("楼阁上层柱体插入下层的深度"),
        )# type: ignore
    
    
    # 墙体属性
    railing_list: bpy.props.CollectionProperty(
        type=ACA_data_railing, name=_("栏杆列表")
    ) # type: ignore
    ...
    geshan_list: bpy.props.CollectionProperty(
        type=ACA_data_geshan, name=_("隔扇列表")
    ) # type: ignore 
    paint_style : bpy.props.EnumProperty(
            name = _("彩画样式"),
            description = _("可以切换清和玺等彩画样式"),
    ...
    use_dg :  bpy.props.BoolProperty(
            ...
            name=_("使用斗栱"),
            update=dc.update_dougong,
            description=_("小式建筑可以不使用斗栱，大梁直接坐在柱头"),
        )# type: ignore 
    use_pingbanfang: bpy.props.BoolProperty(
            default=True,
            name=_("使用平板枋"),
            update=dc.update_dougong,
            description=_("在柱头和斗栱之间的一层垫板，明清式建筑一般都会使用"),
        )# type: ignore 
    dg_style : bpy.props.EnumProperty(
            name = _("斗栱类型"),
            description = _("根据建筑等级的不同，斗栱有严格的限制"),
    ...
    dg_extend : bpy.props.FloatProperty(
            name=_("斗栱挑檐"),    # 令拱出跳距离
            default=0.45,
            description = _("斗栱出跳由斗栱模板预先定义，不可修改"),
            min=0.01,
            precision=3,
        )# type: ignore 
    dg_height : bpy.props.FloatProperty(
            name=_("斗栱高度"),    # 取挑檐桁下皮高度
            default=1.0,
            precision=3,
        )# type: ignore 
    ...
    dk_scale:bpy.props.FloatProperty(
            name=_("斗口放大"),    # 斗栱间距
            description = _("为了模仿唐宋建筑风格，可以放大斗栱"),
            default=1,
            precision=3,
            min=0.5,
            max=2.5,
    ...
    dg_withbeam:bpy.props.BoolProperty(
            name=_("斗栱与大梁连做"),    # 斗栱间距
            description = _("斗栱中已经包含大梁，则不再生成大梁"),
            default=True,
        )# type: ignore 
    ...
    roof_style : bpy.props.EnumProperty(
            ...
            description=_("请选择一种屋顶样式"),
        ) # type: ignore
    use_double_eave: bpy.props.BoolProperty(
            default=False,
            name=_("使用重檐"),
            update=dc.update_roof,
            description=_("使用重檐形式的屋顶"),
        )# type: ignore 
    use_hallway : bpy.props.BoolProperty(
            default=False,
            name=_("做廊步架"),
            update=dc.update_building,
            description=_("在前后廊和周围廊做法时，升高金柱到下金桁高度"),
        )# type: ignore 
    rafter_count : bpy.props.IntProperty(
            ...
            min=2,max=9,
            update=dc.update_roof,
            description=_("以通进深除以22斗口来估算，过大过小会有很多潜在问题"),
        )# type: ignore 
    use_flyrafter :  bpy.props.BoolProperty(
            default=True,
            name=_("使用飞椽"),
            update=dc.update_roof,
            description=_("小式的硬山、悬山可以不做飞椽，但四坡面必须使用飞椽做翼角"),
        )# type: ignore 
    use_wangban :  bpy.props.BoolProperty(
            default=True,
            name=_("添加望板"),
            update=dc.update_roof,
            description=_("可以不做望板，更直观的查看屋顶结构"),
        )# type: ignore 
    qiqiao: bpy.props.FloatProperty(
            ...
            min=0,
            update=dc.update_roof,
            description=_("常做4椽起翘，也可以视情况适当增加"),
        )# type: ignore 
    chong: bpy.props.FloatProperty(
            ...
            min=0, 
            update=dc.update_roof,
            description=_("常做3椽出冲，也可以视情况适当增加"),
        )# type: ignore 
    use_pie: bpy.props.BoolProperty(
            name=_("使用撇"),
            default=True,
            update=dc.update_roof,
            description=_("翼角翘飞椽可以选择是否做官式的撇向做法，起翘夸张的非官式做法建议关闭"),
    )# type: ignore
    ...
    liangtou: bpy.props.FloatProperty(
            ...
            precision=3,
            update=dc.update_roof,
            description=_("老梁头压挑檐桁的尺度，建议在0.5左右，可根据起翘形态适当调整")
        )# type: ignore
    tuishan: bpy.props.FloatProperty(
            ...
            precision=3,
            update=dc.update_roof,
            description=_("庑殿顶两山坡度的调整系数，标准值为0.9，设置为1.0即不做推山")
        )# type: ignore
    shoushan: bpy.props.FloatProperty(
            ...
            precision=3,
            update=dc.update_roof,
            description=_("歇山顶的山花板从檐檩中向内移动的距离(米)，一般为1檩径(4斗口)，最大不超过檐步架")
        )# type: ignore
    luding_rafterspan:bpy.props.FloatProperty(
            ...
            precision=3,
            update=dc.update_roof,
            description=_("盝顶檐步架宽度，用于重檐时，请设置为上下层面阔/进深收分的距离")
        )# type: ignore
    juzhe : bpy.props.EnumProperty(
            name = _("举折系数"),
            # 枚举项仅国际化显示文本，内部值(0~3)保持不变
            items = [
                ("0",_("   举折系数：默认"),_("[0.5,0.7,0.8,0.9]")),
                ("1",_("   举折系数：陡峭"),_("[0.5,1,1.5,2]，慎用，一般用于亭子等建筑")),
                ("2",_("   举折系数：平缓"),_("[0.5,0.65,0.75,0.9]")),
                ("3",_("   举折系数：按屋架高度推算"),_("根据输入屋架高度，进行举折计算"))
            ],
            description=_("决定了屋面坡度的曲率"),
    ...
    tile_scale:bpy.props.FloatProperty(
            ...
            min=0.5,max=2.0,
            precision=2,
            description=_("放大或缩小瓦作的比例，默认1.0"),
            update=dc.update_building,
        )# type: ignore
    ...
    is_4_sides:bpy.props.BoolProperty(
            default = True,
            name = _("是否做四面墙"),
            description=_("同时生成四面合围的墙体，转角处将做45度拼接"),
        ) # type: ignore
    ...
    juzhe_var :bpy.props.FloatProperty(
            ...
            max=0.3,
            update=dc.update_roof,
            description=_("举折系数，默认0.1，适当调大可以获得更加明显的瓦面弧度"),
        )# type: ignore
    
    # 251205 后处理操作列表
    postProcess: bpy.props.CollectionProperty(
        type=ACA_data_postProcess, name=_("后处理列表")
    ) # type: ignore
