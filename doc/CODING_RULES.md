...
### 11.1 存储位置规范
- **强制路径**：所有生成的文档（包括但不限于需求文档、技术方案、重构日志、API说明等）必须保存在 `doc/` 目录下。
- **目录结构**：建议根据文档类型在 `doc/` 下创建子目录（如 `doc/重构/`、`doc/API/`），保持结构清晰。
- **禁止项**：严禁在项目根目录或 Python 代码包目录（如 `ACA Builder/`）下直接生成 `.md` 或 `.txt` 文档文件，以免污染代码结构。

### 11.2 语言使用规范
- **默认语言**：文档内容默认采用 **中文** 编写。
- **混合使用**：
    - 允许并鼓励在描述代码实体时保留英文原文（如变量名、类名、API 路径）。
    - 专有名词（如 Blender Operator、Panel）可保留英文或使用“英文(中文)”格式。
    - 示例：`class ACA_OT_add_building`（无需翻译类名）。

### 11.3 命名规范
- **技术债分析文档**：统一命名格式为 `{模块名}_技术债分析.md`。
    - 示例：`data_技术债分析.md` (对应 `data.py`)
    - 示例：`buildFloor_技术债分析.md` (对应 `buildFloor.py`)
    - 示例：`tools_aca_logging_技术债分析.md` (对应 `tools/aca_logging.py`)
    - 目的：保持文档列表的整洁和统一，便于检索。
...
### 12.1 操作权限控制
- **必须提示**：设计所有 Git 相关操作（commit, push, revert, merge 等）前，必须先向用户提示并解释操作意图。
- **明确授权**：只有在用户给出明确执行指令（如“可以”、“执行”、“提交”等）后，方可执行 Git 命令。
- **严禁自动**：严禁在未获明确授权的情况下，自行后台执行代码签入（check-in）或修改版本库状态的操作。
- **命令审查**：在申请操作权限时，应展示拟执行的完整 Git 命令，供用户审查。
