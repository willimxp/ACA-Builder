...
class wallBuilder(object):
    ...
    # 返回net_x[],net_y[]数组
    def __getFloorDate(self,buildingObj:bpy.types.Object):
        # 载入设计参数
    ...
    # 创建新地盘对象（empty）
    def __addWallrootNode(self,buildingObj:bpy.types.Object):
        # 创建新地盘对象（empty）
    ...
    # 计算墙体数据
    def __getWallData(self,buildingObj:bpy.types.Object,net_x,net_y):
    ...
    def updateWallLayout(self,buildingObj:bpy.types.Object):
        ...
        # b、计算布局数据
        net_x,net_y = self.__getFloorDate(buildingObj)
        row,col,rowRange,colRange = \
            self.__getWallData(buildingObj,net_x,net_y)  
        wallcounter = 0       
        # c、生成横向墙体
        for r in row: 
            for c in colRange:
    ...
    def buildWallLayout(self,buildingObj:bpy.types.Object) :
        # 校验输入对象
        bData : data.ACA_data_obj = buildingObj.ACA_data
        ...
        # 如果找不到“墙体布局”根节点，重新创建
        if wallrootObj == None:        
            wallrootObj = self.__addWallrootNode(buildingObj)

        # 判断墙体数量是否变化
        net_x,net_y = self.__getFloorDate(buildingObj)
        row,col,rowRange,colRange = self.__getWallData(buildingObj,net_x,net_y)
        # 原有墙体数量
