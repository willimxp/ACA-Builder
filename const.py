...
class ACA_ConstsMeta(type):
    """元类，用于拦截类属性的访问并实现动态翻译"""
    ...
    def __getattribute__(cls, name):
        if name in ('_I18N_KEYS', '__dict__', '__mro__', '__bases__'):
            return type.__getattribute__(cls, name)
        
        i18n_keys = type.__getattribute__(cls, '_I18N_KEYS')
        
        if name in i18n_keys:
            raw_value = type.__getattribute__(cls, name)
            return _(raw_value)
        
        return type.__getattribute__(cls, name)


class ACA_Consts(object, metaclass=ACA_ConstsMeta):
    ...
    BOFENG_OFFSET_XSS = 0   # 博缝板下缘距离桁下皮的距离(DK)，歇山四架
    ...
    M_STONE = 'Stone'
    M_WOOD = 'Wood'
    M_BRICK_WALL = 'BrickWall'
    M_PLATFORM_FLOOR = 'PlatformFloor'
    M_PLATFORM_WALL = 'PlatformWall'
    M_PLATFORM_ROCK = 'PlatformRock'
    M_PLATFORM_EXPAND = 'PlatformExpand'
    M_QUETI = 'Queti'
    M_FANG_CHUANCHA = 'Chuancha-Fang'
    M_FANG_JIN = 'JinFang'
    M_FANG_EBIG = 'EbigFang'
    M_FANG_ESMALL = 'SmallFang'
    M_BOARD_YOUE = 'You-E-Board'
    M_BOARD_WALLHEAD = 'WallHeadBoard'
    M_PILLAR_BASE = 'PillarBase'
    M_PILLAR = 'Pillar'
    M_PILLAR_HEAD = 'PillarHead'
    M_WALL_BOTTOM = 'WallBottom'
    M_WALL = 'Plaster'
    M_WINDOW = 'Window'
    M_WINDOW_INNER = 'WindowInner'
    M_WINDOW_WALL = 'WindowWall'
    M_DOOR_RING = 'DoorRing'
    M_DOOR_BOTTOM = 'DoorBottom'
    M_FANG_PINGBAN = 'FangPingban'
    M_FANG_TIAOYAN = 'FangTiaoyan'
    M_FANG_DGCONNECT = 'FangDgConnect'
    M_DOUGONG = 'DouGong'
    M_BOARD_DG = 'BoardDg'
    M_BOARD_DG_S = 'BoardDgS'
    M_BAWANGQUAN = 'Bawangquan'
    M_BEAM_NOPAINT = 'BeamNoPaint'
    M_BEAM_PAINT = 'BeamPaint'
    M_ROOF_PAINT = 'RoofPaint'
    M_ROOF_NOPAINT = 'RoofNoPaint'
    M_CORNERBEAM = 'CornerBeam'
    M_CORNERBEAM_S = 'SubCornerBeam'
    M_RAFTER = 'Rafter'
    M_FLYRAFTER = 'FlyRafter'
    M_WANGBAN = 'Sheathing'
    M_SHANHUA = 'ShanHua'
    M_XIANGYAN = 'XiangYan'
    M_ZHILINGCHUANG = 'ZhiLingChuang'
    M_MENZAN = 'MenZan'
    M_LINXIN_WAN = 'LinXinWan'
    M_RAILING = 'Railing'
    M_GUAYANBAN = 'GuayanBan'
